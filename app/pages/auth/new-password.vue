...
async function handleSubmit() {
  ...
  try {
    await $fetch('/api/auth/new-password', {
      method: 'POST',
  ...
  >
    <form class="space-y-6" @submit.prevent="handleSubmit">
      ...
      <NuxtLink
        v-if="success"
        to="/auth/login"
        class="inline-flex items-center justify-center rounded-md text-sm font-medium h-10 px-4 py-2 w-full border border-input bg-background hover:bg-accent hover:text-accent-foreground"
      >
        Go to Login
      </NuxtLink>
    </form>
